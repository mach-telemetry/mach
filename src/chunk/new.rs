...
impl<const H: usize, const T: usize> Inner<H, T> {
    ...
    fn push(&mut self, segment: &FullSegment) -> Option<&mut FlushBuffer<H, T>> {
        let (mint, maxt) = {
...
impl<const H: usize, const T: usize> WriteChunk<H, T> {
    pub fn push(&mut self, segment: &FullSegment) -> Option<&mut FlushBuffer<H, T>> {
    ...
    pub fn flush_buffer(&mut self) -> &mut FlushBuffer<H, T> {
...
mod test {
    ...
    fn serialized() {
        ...
        let bytes = chunk_writer.flush_buffer().data();
