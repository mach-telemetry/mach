...
use std::sync::{Arc, Mutex, RwLock, atomic::{AtomicUsize, Ordering::SeqCst}};
use std::sync::mpsc;
...
async fn main()  -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.tsdb.as_str() {
        ...
        "file" => {
            let processor = FileOtlpProcessor::new(args.file_path.as_ref().unwrap().clone(), args.file_item_count);
            let server = OtlpServer { processor };
            let trace_server = TraceServiceServer::new(server);
            Server::builder()
                .add_service(trace_server)
                //.add_service(MetricsServiceServer::new( server.clone() ))
